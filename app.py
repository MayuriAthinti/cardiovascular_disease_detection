...
@app.route("/notebook1")
def notebook1():
    return render_template("HHDD.html")

@app.route("/notebook2")
def notebook2():
    return render_template("BRFSS.html")
...
def predict():
    ...
    model = joblib.load('models/hhdd/model_anova.sav')
    predict = model.predict(final4)

    if predict == 0:
        output = "PREDICTED AS PATIENT WITH NO HEART DISEASE!" 
    elif predict == 1:
        output = "PREDICTED AS PATIENT WITH HEART DISEASE!" 
...
def predict1():
    ...
    print(int_features,len(int_features))
    final4=[np.array(int_features)]
    model = joblib.load('models/brfss/model_anova.sav')
    predict = model.predict(final4)
